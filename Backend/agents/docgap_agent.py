...
def available_documents(profile: CitizenProfile) -> set[str]:
    """Return the set of document keys the citizen has (value is True)."""
    gov = profile.gov_id_available
    return {doc for doc, has in gov.model_dump().items() if has is True}


def missing_documents_for_scheme(profile: CitizenProfile, scheme: Scheme) -> list[str]:
    ...
    """
    have = available_documents(profile)
