...
import { submitIntake, type CitizenProfile } from "@/lib/api";
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    ...
    land_owned_acres: "0", education_level: "",
  });
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select className="block w-full rounded-lg border-2 border-ink-navy/15 bg-white px-3.5 py-2.5 text-sm text-ink-navy focus:border-signal-orange focus:outline-none" value={form.occupation} onChange={(e) => update("occupation", e.target.value)}>
                      <option value="">Select</option>
                      <option>Farmer</option><option>Agricultural labourer</option>
                      <option>Daily wage worker</option><option>Government employee</option>
                      <option>Private sector employee</option><option>Self-employed</option>
                      <option>Student</option><option>Homemaker</option><option>Unemployed</option>
                      <option>Retired</option><option>Other</option>
                    </select>
                  </div>
                  <Input label="Education level" placeholder="e.g. Graduate, 10th pass, etc." value={form.education_level} onChange={(e) => update("education_level", e.target.value)} />
